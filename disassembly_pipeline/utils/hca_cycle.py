...
from .disassembly_manager import Disassembly_manager


class Hca_cycle(Disassembly_manager):
    def __init__(self, **kwargs):
        """ 
        !!! Must be provided with a Disassembly_manager superclass !!!
        """

        super().__init__(**kwargs) # initialize Disassembly_manager
    ...
    #TODO: Move this method into a separate script and move the other shit into skills
    ...
    def p1_pickup_hca_and_put_into_vice(self, init_safe = True, double_tap_vise = False, ignore_bad_rotation = False, stop_after_raising = False, return_to_via = False, hca_x= None):
        ...
        # PICKUP HCA PARAMS
        dT_grip_kalo = self.parameters['hca_kalo'][robot.gripper]['dT_grip'] # Long HCA
        dT_grip_qundis = self.parameters['hca_qundis'][robot.gripper]['dT_grip'] # Short hca

        dT_vise_drop_kalo = self.parameters['hca_kalo'][robot.gripper]['dT_vise_drop'] # Long HCA
        dT_vise_drop_qundis = self.parameters['hca_qundis'][robot.gripper]['dT_vise_drop'] # Long HCA
        ...
        if hca_x is not None:
            ...
            if hca_type == 'kalo': dT_grip = dT_grip_kalo
            elif hca_type == 'qundis': dT_grip = dT_grip_qundis
            if T_hca is None: 
        ...
        T_hca_grip = T_hca@dT_grip
        
        x_pickup = t2x(T_hca_grip)
        
        x_above_pickup = copy.deepcopy(x_pickup)
        x_above_pickup[2] += init_height_above_hca
        ...
        do_concurrently([[robot.CMove, {'x' : x_above_pickup, 't' : 5 / self.sf, 'v_max_factor': 0.4, 'a_max_factor':0.4}]], wait_until_task_completion = True)

        robot.CMove(x = x_pickup, t = 3.3 / self.sf, task_space ='World')
